...
var rubySassOptions = {'sourcemap=none': true, verbose: false, stopOnError:true};
...
gulp.task('build:css', function() {
    return $.rubySass('./src/date-picker/date-picker.scss',rubySassOptions).on('error', function (err) {
        console.error('Error!', err.message);
    })
        .pipe($.rename('angular-material-components.css'))
        .pipe(gulp.dest('./dist'));
});

gulp.task('build:css-min', function() {
    return $.rubySass('./src/date-picker/date-picker.scss',rubySassOptions)
        .pipe($.rename('angular-material-components.min.css'))
        .pipe($.csso())
        .pipe(gulp.dest('./dist'));
});
